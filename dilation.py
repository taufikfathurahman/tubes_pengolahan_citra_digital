...
class Dilation:
    def __init__(self, binary_img_matrix, structuring_element=np.ones((3, 3))):
        ...
        self.dilated_img = np.zeros((binary_img_matrix.shape[0], binary_img_matrix.shape[1]))
        ...
        for i in range(len(self.binary_img_matrix)):
            for j in range(len(self.binary_img_matrix[0])):
                ...
                if shp[0] != 0 and shp[1] != 0 and np.logical_and(
                        self.structuring_element[ste_first_row_idx:ste_last_row_idx + 1,
                        ste_first_col_idx:ste_last_col_idx + 1], overlap).any():
                    self.dilated_img[i, j] = 1

        output = (self.dilated_img * 255).astype("uint8")
